...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    api.get('/admin/order-locations')
      ...
      .catch(() => toast.error(t('admin.analytics.load_error'), {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-text-primary">{t('admin.analytics.title')}</h1>
        ...
        {!loading && (
          <div className="flex items-center gap-2 bg-surface-card border border-neutral-border rounded-sm px-4 py-2">
            ...
            <span className="text-sm text-text-muted">{t('admin.analytics.pinned', { count: locations.length })}</span>
          </div>
        )}
      </div>
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center h-full text-text-muted gap-2">
            ...
            <span className="text-sm">{t('admin.analytics.loading')}</span>
          </div>
        ) : (
      ...
      {!loading && locations.length === 0 && (
        <p className="text-sm text-text-muted text-center py-4">
          {t('admin.analytics.no_locations')}
        </p>
      )}
